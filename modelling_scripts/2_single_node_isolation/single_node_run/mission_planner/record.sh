#!/bin/bash
# Recording script for: mission_planner
# Package: mission_planner
# Executable: mission_planner_node
# Namespace: /autoware
# Latency: 8.4 ms

set -e

SCRIPT_DIR="$(cd -- "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PARENT_DIR="$(dirname "$(dirname "${SCRIPT_DIR}")")"

# Source environments
source /opt/ros/humble/setup.bash
source ${HOME}/autoware/install/setup.bash

# Set remapping file path (empty for now - can be customized)
REMAP_FILE=""

echo "Recording node: mission_planner"
echo "Package: mission_planner"
echo "Executable: mission_planner_node"
echo "Namespace: /autoware"

# Navigate to replayer directory
cd "/home/tristan-toye/ros2_single_node_replayer"

# Start recording
# Format: python3 recorder.py <package> <executable> <namespace> <node_name> <remap_file>
python3 recorder.py "mission_planner" "mission_planner_node" "/autoware" "mission_planner" "${REMAP_FILE}"

echo ""
echo "Recording complete for mission_planner"
echo "Output saved to: /home/tristan-toye/ros2_single_node_replayer/output/"
