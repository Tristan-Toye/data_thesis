...
def load_data(csv_path: str) -> pd.DataFrame:
    ...
    pub_cols = [
        ...
        "pub_latency_std_us", "pub_latency_p50_us", "pub_latency_p95_us",
        "pub_latency_p99_us",
    ]
    for col in latency_cols + pub_cols:
...
def plot_publish_violin_all_nodes(df: pd.DataFrame, output_dir: str):
    """Violin plot using start-to-publish latency (pub_latency_mean_us)."""
    if "pub_latency_mean_us" not in df.columns:
        print("  SKIP: pub_latency_mean_us not in columns for publish violin plot")
        return
    ...
    node_order = sorted(
        nodes,
        key=lambda n: df[df["node"] == n]["pub_latency_mean_us"].median()
        if not df[df["node"] == n]["pub_latency_mean_us"].dropna().empty
        else 0,
        reverse=True,
    )

    data_per_node = [
        df[df["node"] == n]["pub_latency_mean_us"].dropna().values for n in node_order
    ]
    valid_pairs = [(n, d) for n, d in zip(node_order, data_per_node) if len(d) > 0]
    if not valid_pairs:
        print("  WARNING: No data for publish violin plot")
        plt.close()
        return

    valid_nodes, data_per_node = zip(*valid_pairs)
    ...
    short_names = [n.replace("_", "\n") for n in valid_nodes]
    ax.set_xticklabels(short_names, rotation=45, ha="right", fontsize=9)
    ax.set_ylabel("Start-to-Publish Latency (µs)")
    ax.set_title("Parameter Sensitivity: Start-to-Publish Latency Across All Parameter Sweeps",
                 fontsize=14, fontweight="bold", pad=15)
    ax.grid(axis="y", alpha=0.3)
    ...
    fig.tight_layout()
    fig.savefig(os.path.join(output_dir, "violin_publish_all_nodes.png"), bbox_inches="tight")
    plt.close()
    print("  Generated: violin_publish_all_nodes.png")
...
def plot_tornado(df: pd.DataFrame, output_dir: str):
    ...
    for node in df["node"].unique():
        ...
        for param in params:
            param_df = node_df[node_df["parameter"] == param].sort_values("value")
            if "pub_latency_mean_us" in param_df.columns:
                latencies = param_df["pub_latency_mean_us"].values
            else:
                latencies = param_df["latency_mean_us"].values
        ...
        ax.set_title(f"Tornado Chart (Start-to-Publish if available): {node}\n"
...
def plot_latency_vs_param(df: pd.DataFrame, output_dir: str):
    ...
    for node in df["node"].unique():
        ...
        for i, param in enumerate(params):
            ...
            metric = "pub_latency_mean_us" if "pub_latency_mean_us" in param_df.columns else "latency_mean_us"
            latencies = param_df[metric].values
        ...
        ylabel = "Mean Start-to-Publish Latency (µs)" if "pub_latency_mean_us" in df.columns else "Mean Callback Latency (µs)"
        ax.set_ylabel(ylabel)
        ax.set_title(f"Latency vs Parameter Values (start-to-publish if available): {node}",
                     fontsize=13, fontweight="bold", pad=10)
...
def plot_pmu_correlation(df: pd.DataFrame, output_dir: str):
    ...
    for i, col in enumerate(available_pmu):
        ax = axes[i]
        metric = "pub_latency_mean_us" if "pub_latency_mean_us" in df.columns else "latency_mean_us"
        valid = df[[col, metric]].dropna()
        valid = valid[(valid[col] > 0) & (valid[metric] > 0)]
        ...
        for node in valid.merge(df[["node"]], left_index=True, right_index=True)["node"].unique():
            ...
            if len(node_data) > 0:
                ax.scatter(node_data[col], node_data[metric],
        ...
        ax.set_title(f"{metric} vs {pmu_cols[col]}", fontsize=10, fontweight="bold")
        ax.grid(alpha=0.3)

    for j in range(i + 1, len(axes)):
        axes[j].set_visible(False)

    fig.suptitle("PMU Counter Correlations with Latency (start-to-publish if available)",
...
def plot_normalized_sensitivity(df: pd.DataFrame, output_dir: str):
    ...
    for (node, param), group in df.groupby(["node", "parameter"]):
        ...
        metric = "pub_latency_mean_us" if "pub_latency_mean_us" in sorted_g.columns else "latency_mean_us"
        latencies = sorted_g[metric].values
...
def main():
    ...
    parser.add_argument("--input", "-i",
                        default=str(Path(__file__).parent.parent / "experiments" /
                                    "8_start_to_publish" / "tables" / "raw_results_start_to_publish.csv"),
                        help="Path to Experiment 8 enriched CSV")
    parser.add_argument("--output-dir", "-o",
                        default=str(Path(__file__).parent.parent / "experiments" /
                                    "8_start_to_publish"),
                        help="Base output directory for Experiment 8")
    ...
    summary_path = os.path.join(table_dir, "parameter_sensitivity_start_to_publish.csv")
    ...
    plot_publish_violin_all_nodes(df_valid, graph_dir)
